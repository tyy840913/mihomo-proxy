...
# 安装必要的软件包
install_packages() {
    echo -e "${CYAN}正在安装必要的软件包...${PLAIN}"
    apt-get update -qq
    apt-get install -y -qq curl wget jq iproute2
    echo -e "${GREEN}软件包安装完成${PLAIN}"
}
...
check_docker() {
    ...
    echo -e "${YELLOW}Docker未安装，正在安装...${PLAIN}"
...
create_macvlan_network() {
    ...
    if docker network ls | grep -q "mnet"; then
        ...
        docker network rm mnet
    fi
    
    # 创建新的macvlan网络
    docker network create -d macvlan \
        --subnet=$(ip route | grep $MAIN_INTERFACE | grep src | awk '{print $1}') \
        --gateway=$(ip route | grep default | awk '{print $3}') \
        -o parent=$MAIN_INTERFACE \
        mnet
    
    echo -e "${GREEN}Docker macvlan网络创建完成${PLAIN}"
}
...
prepare_mihomo_config() {
    ...
    echo -e "${GREEN}Mihomo配置目录准备完成${PLAIN}"
}
...
start_mihomo_container() {
    ...
    if docker ps -a | grep -q "mihomo"; then
        echo -e "${YELLOW}发现已存在的Mihomo容器，将重新创建...${PLAIN}"
        docker stop mihomo
        docker rm mihomo
    fi
    ...
    docker run -d \
        ...
        --network mnet \
        --ip $MIHOMO_IP \
        -v $CONF_DIR:/etc/mihomo \
        -p 7890:7890 \
        -p 7891:7891 \
        -p 7892:7892 \
        -p 9090:9090 \
        --privileged \
        ghcr.io/mihomo/mihomo:latest
    
    echo -e "${GREEN}Mihomo容器启动完成${PLAIN}"
}
...
main() {
    ...
    echo -e "${CYAN}控制面板地址: http://$MIHOMO_IP:9090${PLAIN}"
    echo -e "${CYAN}代理端口: 7890(混合), 7891(HTTP), 7892(SOCKS5)${PLAIN}"
}
